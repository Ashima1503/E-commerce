...
//   if(req.session.loggedin){
//     let sqlQuery = "SELECT orders.id,orders.p_id,orders.usr_buy,orders.o_id,orders.addr,orders.tracking ,orders.tracking_id,products.name,products.price,products.sale_price,orders.mode FROM orders INNER JOIN products ON products.id = orders.p_id WHERE orders.usr_buy='"+req.session.username+"'";
...
//   const product = getProductById(p_id);
//   const cartItem = {
//     id: p_id,
//     name: product.p_name,
...
//   const totalPrice = cart.reduce((total, results) => total + results.total, 0);
